...
static vec2 ndc_to_pixels(vec2f ndc) {
    return (vec2) {
...
static void draw_rect(vec2f v1, vec2f v2, u32 color) {
    vec2 v1_pixels = ndc_to_pixels(v1);
    vec2 v2_pixels = ndc_to_pixels(v2);
...
static void draw_line(vec2f v1, vec2f v2, u32 color) {
    vec2 v1_pixels = ndc_to_pixels(v1);
    vec2 v2_pixels = ndc_to_pixels(v2);
...
static i32 edge_function(vec2 a, vec2 b, vec2 c) {
    return ((c.x - a.x) * (b.y - a.y) - (c.y - a.y) * (b.x - a.x));
}

static void draw_triangle_raw(i32 x1, i32 y1, i32 x2, i32 y2, i32 x3, i32 y3, u32 color) {
    ...
    e_row2 = edge_function((vec2){x3, y3}, (vec2){x2, y2}, (vec2){min_x, min_y});
    e_row3 = edge_function((vec2){x2, y2}, (vec2){x1, y1}, (vec2){min_x, min_y});
...
static void draw_triangle(vec2f v1, vec2f v2, vec2f v3, u32 color) {
    vec2 v1_pixels = ndc_to_pixels(v1);
    vec2 v2_pixels = ndc_to_pixels(v2);
    vec2 v3_pixels = ndc_to_pixels(v3);
